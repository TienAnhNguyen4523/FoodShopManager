...
namespace BTLlttq
{
	public partial class Admin : Form
	{
		...
		private void btnThemCategory_Click(object sender, EventArgs e)
		{
			...
			int row = dtBase.BoolChangeData("insert FoodCategory(name) values('" + name + "')");
			if (row > 0)
			{
				MessageBox.Show("them thanh cong");
				LoadLstCategory();
		...
		private void btnXoaCategory_Click(object sender, EventArgs e)
		{
			...
			if (row > 0)
			{
				MessageBox.Show("xoa thanh cong");
				LoadLstCategory();
				deletCategory(this, new EventArgs());
			}
			else
			{
				MessageBox.Show("that bai");
